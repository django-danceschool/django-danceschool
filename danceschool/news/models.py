...
class NewsItem(models.Model):
    ...
    title = models.CharField(
        ...
        help_text=_('Title the news item. Make sure this is descriptive.')
    )

    content = HTMLField(_('Content'), help_text=_('Insert news content here.'))

    alert = models.BooleanField(
        _('Alert'), default=False,
        help_text=_(
            'Alerts (such as cancellations) may be displayed in an emphasized fashion.'
        )
    )

    pinThis = models.BooleanField(
        _('Pinned'), default=False,
        help_text=_(
            'If this field is set, then the news item will continue to ' +
            'show up on the main page until someone unpins it.'
        )
    )

    draft = models.BooleanField(
        _('Draft status'), default=False,
        help_text=_('Check to hide from publication')
    )
    hideThis = models.BooleanField(
        _('Hidden'), default=False,
        help_text=_(
            'If for some reason you need to make a news item invisible, ' +
            'this will do that.'
        )
    )
...
class LatestNewsPluginModel(CMSPlugin):
    ...
    daysBefore = models.PositiveSmallIntegerField(
        _('Published since (days ago)'),
        help_text=_('Leave blank for no limit.'), null=True, blank=True
    )
    alertOnly = models.BooleanField(_('Show Alerts Only'), default=False)
    ignorePins = models.BooleanField(
        _('Ignore Pinned Item Precedence'),
        help_text=_(
            'By default, pinned items are shown first, regardless of ' +
            'publication date. Checking this box overrides that behavior.'
        ), default=False
    )
