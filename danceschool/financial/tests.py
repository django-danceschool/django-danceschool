...
class FinancialSummariesTest(DefaultSchoolTestCase):
    ...
    def test_annual_detailview(self):
        ...
        response = self.client.get(reverse('financialYearDetailView'),{'year': ensure_localtime(timezone.now()).year})
        self.assertEqual(response.status_code, 302)
        self.client.login(username=self.superuser.username,password='pass')
        response = self.client.get(reverse('financialYearView'),{'year': ensure_localtime(timezone.now()).year})
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse('financialYearDetailView'),{'year': ensure_localtime(timezone.now()).year, 'basis': 'paymentDate'})
        self.assertEqual(response.status_code, 200)
        self.assertIn(ei, response.context_data.get('otherExpenseItems'))
        self.assertIn(ri, response.context_data.get('otherRevenueItems'))
    ...
    def test_monthly_detailview(self):
        ...
        response = self.client.get(reverse('financialMonthDetailView'),{'year': ensure_localtime(timezone.now()).year, 'month': ensure_localtime(timezone.now()).month})
        self.assertEqual(response.status_code, 302)
        self.client.login(username=self.superuser.username,password='pass')
        response = self.client.get(reverse('financialMonthDetailView'),{'year': ensure_localtime(timezone.now()).year, 'month': ensure_localtime(timezone.now()).month})
        self.assertEqual(response.status_code, 200)
        ...
        # Change the basis to payment/received basis and ensure
        # that the items still show up
        response = self.client.get(reverse('financialMonthDetailView'),{'year': ensure_localtime(timezone.now()).year, 'month': ensure_localtime(timezone.now()).month, 'basis': 'paymentDate'})
        self.assertEqual(response.status_code, 200)
        self.assertIn(ei, response.context_data.get('otherExpenseItems'))
        self.assertIn(ri, response.context_data.get('otherRevenueItems'))
